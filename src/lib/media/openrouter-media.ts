...
export class OpenRouterMediaService implements MediaService {
  ...
  async generateImage(prompt: string, options?: ImageGenerationOptions): Promise<GenerationResult> {
    ...
    const requests = Array.from({ length: variants }, async () => {
      const controller = new AbortController()
      const timeoutId = setTimeout(() => {
        controller.abort()
      }, OpenRouterMediaService.REQUEST_TIMEOUT_MS)
      ...
      try {
        ...
        if (error instanceof Error && error.name === "AbortError") {
          throw new Error("OpenRouter image generation timed out after 30 seconds")
        }
        throw error
      } finally {
    ...
    for (const settledResponse of settledResponses) {
      if (settledResponse.status === "rejected") {
        console.error("[OpenRouterMediaService] Image request failed:", settledResponse.reason)
        continue
      }
    ...
    if (allImages.length === 0) {
      throw new Error(
        "OpenRouter konnte innerhalb des Timeouts kein Bild liefern. Bitte erneut versuchen oder ein anderes Modell waehlen."
      )
    }
