...
const useFileSrc = (file: File | undefined) => {
  ...
  useEffect(() => {
    // Early return if no file - cleanup will handle state reset
    if (!file) return

    const objectUrl = URL.createObjectURL(file)
    // eslint-disable-next-line react-hooks/set-state-in-effect -- Required for object URL sync: creates URL and must update state immediately
    setSrc(objectUrl)
      ...
      >
        <AttachmentPreviewDialog>
          <TooltipTrigger asChild>
            <div
              className={cn(
                "aui-attachment-tile bg-muted size-14 cursor-pointer overflow-hidden rounded-lg border transition-opacity hover:opacity-75",
